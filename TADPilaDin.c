...
NOTA de colin : Se inicializa la variable size para regresar el tamaño de la pila. 
*/
void Initialize(pila *s)
{
	s->tope=NULL; //(*s).tope=NULL;
	(*s).size=0; // se pone el contador en cero
	return;
}
...
void Push(pila *s, elemento e)
{
	...
	aux->abajo=s->tope;
	s->tope=aux;
	// Añadiendo un contador que llevara la cuenta de los push;
	s->size++;
	return;
}
...
elemento Pop (pila *s)
{
	...
	s->tope=s->tope->abajo;
	free(aux);
	s->size--;// le restamos uno a la pila , ta que se elimino un elemento de ella
	return r;	
}
...
int Size(pila *s)
{
	/*Este codigo es por si no tenemos la variable Size.
	nodo *aux;
	int tam_pila=0;
	aux=s->tope;
	if(aux!=NULL)
...
{
	nodo *aux;
	s->size = 0 ; // Se regresa a cero la variable del tamaño
	if(s->tope!=NULL)
	{
		while(s->tope!=NULL)
		{				
